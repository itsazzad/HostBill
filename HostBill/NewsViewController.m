...
- (void)viewDidLoad
{
    ...
    self.string1 = [NSString stringWithFormat:@"%@&call=getNews", string];
    NSLog(@"%@",string1);
}
...
-(void)getData
{
    ...
    if (jsonData) {
        ...
        _tempobject = [jsonObjects valueForKey:@"news"];
        NSLog (@"%@ ",_tempobject);
        ...
        for (int i=0; i < count; i++) {
            ...
            NSString * title = [NSString stringWithFormat:@"%@: %@",[dictionary valueForKey:@"id"], [dictionary valueForKey:@"title"]];
            [_objects insertObject:title atIndex:0];
            NSString * description = [NSString stringWithFormat:@"%@",[dictionary valueForKey:@"content"]];
...
{
    UITableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"News list" forIndexPath:indexPath];
