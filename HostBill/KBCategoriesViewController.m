...
#import "KBCategoriesViewController.h"

@interface KBCategoriesViewController (){
...
@implementation KBCategoriesViewController
...
- (void)viewDidLoad
{
    ...
    self.string1 = [NSString stringWithFormat:@"%@&call=getKBCategories", string];
    NSLog(@"%@",string1);
}
...
-(void)getData
{
    ...
    if (jsonData) {
        ...
        _tempobject = [jsonObjects valueForKey:@"categories"];
        _tempobject = [_tempobject valueForKey:@"categories"];
        NSLog (@"%@ ",_tempobject);
        ...
        for (int i=0; i < count; i++) {
            ...
            NSString * description = [NSString stringWithFormat:@"%@",[dictionary valueForKey:@"description"]];
...
{
    UITableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"Knowledgebase categories" forIndexPath:indexPath];
