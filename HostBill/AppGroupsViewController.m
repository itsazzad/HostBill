...
@interface AppGroupsViewController (){
...
- (void)viewDidLoad
{
    ...
    self.string1 = [NSString stringWithFormat:@"%@&call=getAppGroups", string];
    NSLog(@"%@",string1);
}
...
-(void)getData
{
    ...
    if (jsonData) {
        ...
        _tempobject = [jsonObjects valueForKey:@"servers"];
        NSLog (@"%@ ",_tempobject);
        ...
        for (int i=0; i < count; i++) {
            ...
            NSString * description = [NSString stringWithFormat:@"%@",[dictionary valueForKey:@"modulename"]];
...
{
    UITableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"Apps/Servers" forIndexPath:indexPath];
