...
- (void)viewDidLoad
{
    ...
    self.string1 = [NSString stringWithFormat:@"%@&call=getEstimates", string];
    NSLog(@"%@",string1);
}
...
-(void)getData
{
    ...
    if (jsonData) {
        ...
        _tempobject = [jsonObjects valueForKey:@"estimates"];
        NSLog (@"%@ ",_tempobject);
        ...
        for (int i=0; i < count; i++) {
            ...
            NSString * title = [NSString stringWithFormat:@"%@: %@",[dictionary valueForKey:@"id"], [dictionary valueForKey:@"subject"]];
...
{
    UITableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"Estimates" forIndexPath:indexPath];
